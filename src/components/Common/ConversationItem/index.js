...
const ConversationItem = props => {
    ...
    useEffect(() => {
        if(props?.data?._id && !props?.data?.messages){
    ...
    const handleSendMessage =(values)=>{
        ...
        dispatch(addMessage({ recipient,
            _id: props?.data?._id||'',...values}));
    }
      ...
      <ConversationItemWrapper>
                <div className="header">
                    ...
                    <CloseOutlined onClick={()=>{
                        dispatch(toggleConversation(props?.data?._id))
                    }}/>
                    </div>

                </div>
                <div className="body" onScroll={handleOnScrollTop} >
                    {props?.data?.messages?.map((m,index)=> <Message key={index} data={m} recipient={recipient} type={m?.sender === user._id?'me':'friend'}/>)}
                </div>
